...
const area = () => {
  ...
  router.post(
    ...
    body("urn", "urn must be numeric type")
      .if(body("uid").isEmpty())
      .isNumeric(),
    body("uid", "uid must be a valid UUID").if(body("urn").isEmpty()).isUUID(),
    body("type", "type must be an integer").isInt(),
    body("title", "title field cannot be more than 255 characters")
      .isLength({ max: 255 })
      .escape(),
    body("summary", "summary field cannot be more than 340 characters")
      .isLength({ max: 340 })
      .escape(),
    body("body", "body field cannot be more than 5000 characters")
      .isLength({ max: 5000 })
      .escape(),
    body(
      "publishedAt",
    ...
    body("expiresAt", "expiresAt is not a valid ISO8601 format")
