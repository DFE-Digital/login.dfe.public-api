...
describe("When providing invalid body to upsert an announcement via /organisations/announcements", () => {
  const body = {
    ...
    type: 21,
    title:
      "<p>The great announcement. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce ut sapien eros. Aenean dignissim erat ac libero bibendum faucibus. Quisque aliquet auctor sapien, sed dapibus quam. Nullam tempus arcu enim, sit amet consequat ipsum. </p>",
    summary:
      "<div><p>This is a summary. Duis sagittis elit sit amet ante finibus, id fermentum augue condimentum. Mauris tincidunt convallis vehicula. Vivamus vitae elit fringilla, tempus orci nec, ornare sapien. Sed gravida quam non orci vulputate elementum. Morbi eget blandit tellus. Curabitur pharetra est a accumsan auctor. Sed eu rutrum justo.</p></div>",
    body:
      "<div><p>This is a announcement body. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce ut sapien eros. Aenean dignissim erat ac libero bibendum faucibus. Quisque aliquet auctor sapien, sed dapibus quam. Nullam tempus arcu enim, sit amet consequat ipsum rutrum non. Nam non lacus nulla. In et felis non sem interdum vehicula sed nec eros. Nam luctus vitae nibh nec vulputate. In sem eros, molestie id sagittis a, pharetra quis odio. Sed maximus nisi libero, condimentum tincidunt mauris eleifend sed. In sagittis accumsan lorem vitae bibendum. Vivamus leo metus, feugiat vitae velit id, ornare hendrerit ex. Cras ac mi sollicitudin mauris viverra posuere. Nam et porta urna." +
  ...
  it("Response body returns error in reasons object", async () => {
    const response = await request(app)
      ...
      .set(
        "Authorization",
        "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJsb2NhbCIsImF1ZCI6InNpZ25pbi5lZHVjYXRpb24uZ292LnVrIiwic3ViIjoiZjQ0NWViMDctZjI2YS00OWY4LWI3YzgtZDlmZjBmYWVjZTljIn0.9V8qjbDSE_4pwKBL8xJViBQmhPAqc99_ZOXPmle0NLI",
      )
      .send(body)
      .expect(400);

    expect(
      response.body.reasons.includes(
        "body field cannot be more than 5000 characters",
      ),
    ).toBe(true);
    expect(
      response.body.reasons.includes("expiresAt is not a valid ISO8601 format"),
    ).toBe(true);
    expect(
      response.body.reasons.includes("messageId must be a valid UUID"),
    ).toBe(true);
    expect(
      response.body.reasons.includes(
        "publishedAt is not a valid ISO8601 format",
    ...
    expect(
      response.body.reasons.includes(
        "type must be one of 1, 2, 4, 5. Received 21",
      ),
    ).toBe(true);
    expect(response.body.reasons.includes("urn must be numeric type")).toBe(
      true,
    );
  });
});
