...
const { getOrganisationsAssociatedWithUser } = require('../../../src/infrastructure/organisations/api');
...
describe('when getting a users services mapping from api', () => {
  ...
  it('then it should call users resource with user id', async () => {
    await getOrganisationsAssociatedWithUser(userId, correlationId);
  ...
  it('should return null on a 401 response', async () => {
    ...
    const result = await getOrganisationsAssociatedWithUser(userId, correlationId);
  ...
  it('should raise an exception on any failure status code that is not 401, 404 or 409', async () => {
    ...
    const act = () => getOrganisationsAssociatedWithUser(userId, correlationId);
