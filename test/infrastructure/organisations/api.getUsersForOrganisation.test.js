...
const { getUsersForOrganisation } = require('../../../src/infrastructure/organisations/api');
...
describe('when getting a users services mapping from api', () => {
  ...
  it('then it should call users resource with user id', async () => {
    await getUsersForOrganisation(organisationId, correlationId);

    expect(fetchApi.mock.calls).toHaveLength(1);
    expect(fetchApi.mock.calls[0][0]).toBe('http://organisations.test//organisations/org-1/users');
  ...
  it('should return null on a 401 response', async () => {
    ...
    const result = await getUsersForOrganisation(organisationId, correlationId);
  ...
  it('should raise an exception on any failure status code that is not 401, 404 or 409', async () => {
    ...
    const act = () => getUsersForOrganisation(organisationId, correlationId);
