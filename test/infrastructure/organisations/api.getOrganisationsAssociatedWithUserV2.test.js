...
const { getOrganisationsAssociatedWithUserV2 } = require('../../../src/infrastructure/organisations/api');
...
describe('when getting a users services mapping from api', () => {
  ...
  it('then it should call users resource with user id', async () => {
    await getOrganisationsAssociatedWithUserV2(userId, correlationId);

    expect(fetchApi.mock.calls).toHaveLength(1);
    expect(fetchApi.mock.calls[0][0]).toBe('http://organisations.test/organisations/v3/associated-with-user/user-1');
  ...
  it('should return null on a 401 response', async () => {
    ...
    const result = await getOrganisationsAssociatedWithUserV2(userId, correlationId);
  ...
  it('should raise an exception on any failure status code that is not 401, 404 or 409', async () => {
    ...
    const act = () => getOrganisationsAssociatedWithUserV2(userId, correlationId);
