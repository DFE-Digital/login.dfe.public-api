...
describe("when calling the getPoliciesOfService function", () => {
  beforeEach(() => {
    fetchApi.mockReset();
  ...
  it("then it should call users resource with user id", async () => {
    await getPoliciesOfService(serviceId, correlationId);

    expect(fetchApi.mock.calls).toHaveLength(1);
    expect(fetchApi.mock.calls[0][0]).toBe(
      "http://access.test/services/service-1/policies",
    );
  ...
  it("should return false on a 404 response", async () => {
    ...
    const result = await getPoliciesOfService(serviceId, correlationId);
