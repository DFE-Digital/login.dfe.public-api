...
const { getServices } = require("../../../src/infrastructure/access/api");

const serviceId = "service-1";
const correlationId = "abc123";
const apiResponse = {};

describe("when calling the getServices function", () => {
  beforeEach(() => {
    fetchApi.mockReset();
  ...
  it("then it should call users resource with user id", async () => {
    await getServices(serviceId, correlationId);

    expect(fetchApi.mock.calls).toHaveLength(1);
    expect(fetchApi.mock.calls[0][0]).toBe(
      "http://access.test/services/service-1",
    );
  ...
  it("should return false on a 404 response", async () => {
    ...
    const result = await getServices(serviceId, correlationId);
