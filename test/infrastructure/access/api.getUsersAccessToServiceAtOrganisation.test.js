...
describe("when calling the getUsersAccessToServiceAtOrganisation function", () => {
  beforeEach(() => {
    fetchApi.mockReset();
  ...
  it("then it should call users resource with user id", async () => {
    await getUsersAccessToServiceAtOrganisation(
      userId,
      serviceId,
      organisationId,
      correlationId,
  ...
  it("should return false on a 404 response", async () => {
    ...
    const result = await getUsersAccessToServiceAtOrganisation(
      userId,
      serviceId,
      organisationId,
      correlationId,
