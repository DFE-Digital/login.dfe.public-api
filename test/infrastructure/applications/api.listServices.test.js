...
describe("when using the listServices function", () => {
  beforeEach(() => {
    fetchApi.mockReset();
  ...
  it("then it should call services resource", async () => {
    await listServices(serviceId, page, pageSize, correlationId);

    expect(fetchApi.mock.calls).toHaveLength(1);
    expect(fetchApi.mock.calls[0][0]).toBe(
      "http://applications.test/services?page=1&pageSize=10&parent=service-1",
    );
  ...
  it("should return false on a 404 response", async () => {
    ...
    const result = await listServices(serviceId, page, pageSize, correlationId);
