...
const { createService } = require('../../../src/infrastructure/applications/api');
...
describe('when getting a users services mapping from api', () => {
  ...
  it('then it should call users resource with user id', async () => {
    await createService(service, correlationId);

    expect(fetchApi.mock.calls).toHaveLength(1);
    expect(fetchApi.mock.calls[0][0]).toBe('http://applications.test/services');
  ...
  it('should return false on a 404 response', async () => {
    ...
    const result = await createService(service, correlationId);
